function DepartmentsBudget({ budget, summary, departments, productionId, canEditBudget = true, onUpdateAllocation, onRefresh, onAutoAllocate }) {
    ...
    return (
        <div className="space-y-6">
            {/* Smart Allocate */}
            {canEditBudget && (
                <div className="flex items-center justify-between bg-[#fdf5f5] border border-[#e8c5c8] rounded-lg px-4 py-3">
                    <div>
                    ...
                    <div className="smart-alloc-wrap" onMouseDown={e => e.stopPropagation()}>
                        <button
                            ...
                            className="px-3 py-2 bg-[#8B1A2B] hover:bg-[#6e1522] text-white rounded-lg text-sm font-medium transition-colors"
                        >
                            ⚡ Auto Allocate ▾
                        </button>
                        {showTemplates && (
