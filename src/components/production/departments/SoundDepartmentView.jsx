...
function SoundDepartmentView({ production, onUpdateScene, userRole }) {
  ...
  const ghostLightPanel = React.createElement(
    ...
    React.createElement(window.GhostLightLogo, { size: 40, opacity: 0.25, style: { position: 'absolute', right: '24px', bottom: '24px', pointerEvents: 'none' } }),
    React.createElement('div', { style: { marginBottom: '32px' } },
      React.createElement(window.GhostLightLogo, { size: 56, withWordmark: true, style: { marginBottom: '8px' } }),
    ...
    React.createElement(
      ...
      ghostLightFeatures.map((feature, i) =>
        React.createElement('div', { key: i, style: { background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '24px', position: 'relative' } },
          ...
          React.createElement(window.GhostLightLogo, { size: 28, opacity: 0.25, style: { position: 'absolute', bottom: '10px', right: '10px' } })
        )
      )
    )
  );
  ...
  const scenesWithSound = localProduction.acts.reduce((sum, act) => sum + (act.scenes?.filter(s => s.songTitle || s.artist || s.duration || s.soundType)?.length || 0), 0);
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement('span', {
            ...
            React.createElement(window.GhostLightLogo, { size: 36 })
          )
        )
      )
    ),
